...
void Rainbow::tick() {
    ...
    for (char i = 0; i < NUM_BARS/2; i++) {
        int led_index = 0;
        for (char j = 0; j < BAR_HEIGHT; j+=10) {
            // get bin
    ...
    for (char i = 0; i < NUM_BARS; i++) {
        for (char j = 0; j < BAR_HEIGHT/4; j++) {
            ...
            // set bar
            if (j < (pow((float)(level)/(float)(BINS_MAX), 2)) * (BAR_HEIGHT/2)) {
                float val = level*2*PI/BINS_MAX;
                mix_pixel(i, BAR_HEIGHT/2 - j, .95, 255-bd->r(), 255-bd->g(), 255-bd->b());
            }
        }

        // Left bins, grow upwards
        for (char j = BAR_HEIGHT/2; j < BAR_HEIGHT*0.75f; j++) {
            // get bin
            int level = 0;
            switch(i) {
                case 0:
            ...
            if (j-BAR_HEIGHT/2 < (pow((float)(level)/(float)(BINS_MAX), 2)) * (BAR_HEIGHT/2)) {
