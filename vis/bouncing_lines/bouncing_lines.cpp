...
: Visualizer(bars, bins, bd)
{
    ...
    for(short i = 0; i < NUM_BARS; i++) {
        ...
        bouncing_line_lengths[i] = random(10, BAR_HEIGHT / 2);
        bouncing_line_positions[i] = random(0, BAR_HEIGHT);
        //bouncing_line_positions[i] = (i%2 == 0) ? 0 : BAR_HEIGHT;
...
void BouncingLines::tick() {
    for (int i = 0; i < NUM_BARS; i++) {
        ...
        if (bouncing_line_positions[i] < -.5*bouncing_line_lengths[i] && bouncing_line_directions[i] < 0) {
            ...
            if (random(0, 10) > 5) {
                int newlength = random(10, BAR_HEIGHT / 2);
            ...
            bouncing_line_colors[i][1] = bd->g();
            bouncing_line_colors[i][2] = bd->b();
        } else if (bouncing_line_positions[i] > BAR_HEIGHT-.5*bouncing_line_lengths[i] && bouncing_line_directions[i] > 0) {
            bouncing_line_directions[i] = -1;
